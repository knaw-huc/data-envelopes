...
This page contains help documentation, including examples, for the fields in a data-envelope as implemented in the Huygens Data-Envelop editor.

Some fields in a data-envelope are compulsory. If these fields are not filled in, then the data-envelope will fail validation. 
...
The first version number refers to the version of the dataset being documented, the second version number refers to the version of the documentation itself.
...
Please use this space to provide any feedback on this section and to share any other insights or information you think would be useful. For example, you can indicate to what extent the necessary information for the data-envelope was easily available: was this a current project with experts available to answer all the questions, or is it data from fifty years ago for which the description has been pieced together from old documentation sources? 

# Level 2 Basic Metadata
This level describes the basic metadata. This includes: what the dataset contains, what the temporal and geographical coverage are, how it can be accessed etc. 

## 2.1 Snapshot
This contains information about what is in the dataset.

There is some discussion as to what should be included in the temporal and geographical coverage. For example, for a collection of letters it could refer to the locations the letters were sent to/from or the locations discussed in the letters. Try to think about what information is most useful for users of the dataset. 

* Dataset title - the title of the dataset
* Version - the version number of the dataset
* Description - Provide a free text account of the dataset or resource (limit 200 words). Include information about the content and topic of the data, sources and motivations for dataset creation.
* Genre - One or more genres from a vocabulary that apply to the dataset
* Other Genre - Zero or more genres that apply to the dataset that are not included in the vocabulary
* Topic Classification - One or more topics from a vocabulary that apply to the dataset
* Other Topic - Zero or more topics that apply to the dataset that are not included in the vocabulary
...
* Other Language - Zero or more languages that apply to the dataset that are not included in the vocabulary
...
* Other Geographical coverage - Zero or more geographical areas that apply to the dataset that are not included in the vocabulary

### Temporal coverage

This refers to the temporal coverage of the content of the dataset, not the dates on which the dataset was created (these should be entered in section 2.2).
...
## 2.2 Dates
These dates refer to the time span over which the dataset was created. For example, the start and end date of the project to create the data. The date on which the dataset is published can also be specified.
...
### Creators
Creators are responsible for the creation of the dataset. There is no strict definition of the difference between a creator and a contributor. You can include as many creators and contributors as you like.
...
* Current Version - the current version of the dataset
...
* Maintenance Plan - a plan for how the dataset will be maintained
  * Updates - Information about criteria for updating the dataset. Include information on how errors are triaged or handled, and criteria for updating the dataset.
 ...
 * Next planned Update
  ...
  The level represents the Data Resource of the dataset. A Resource is something that a user can utilise to work with the dataset. A Resource can be a downloadable file, or a search portal in which the datsaet can be browsed, or an endpoint for querying the data, etc. The entire level can be repeated. Each repetition represents a Resource within the dataset. Different Resources may be used, for example, to describe different files within the data. They can also be used to describe different variants that are available, for example an open version and a restricted version. However, you can combine multiple files or multiple versions within the same Resource. When deciding how many Resources to use to represent the dataset, consider how much of the information in Level 3 the Resources have in common, how clear the description is and how easy it is for the user to understand what is available to them. 

  ## 3.1 Data Resource Description
  This describes some basic information about the Resource, such as name, description, format and size. Depending on the dataset, some information may be a repetition of information from Level 2, or it may be different. For example, if a dataset is split into different language files, then the Languages in Level 2 may be German, Dutch and French, while the language of the Resource representing the German section will just be German. However, if the all of the Resources were a mix of the three languages then Level 2 and all Resources would have German, Dutch and French for their Languages. 

  * Name - the name of the Resource
  * Description - a description of the particular Resource. This often contains information on how the Resource was created. For example, if a dataset contains a set of persons detected in letters, then it could explain how Named Entity Recognition was used to detect the persons. It also often contains information on how to use the Resource. 
  ...
  * Language(s) - the language(s) used in the Resource
...
* Data modality
    * Modality - one or more data modality (e.g. image, text) selected from a vocabulary
    * Other Modalitie(s) - if the vocabulary does not contain appropriate modalities, then they can be filled in here
* Descriptive statistics - this can be used to describe the Resource with statistics, e.g. how many persons, how many different countries, the distribution over time. It can also be used to link to a page where statistics are maintained

## 3.2 Data Fields
These describe the data fields in the dataset. For example, for a spreadsheet the column and/or row headers could be described. It is also possible to link to a schema that describes the dataset structure, instead of describing the individual data fields.
...
* External Schema Note - Any additional information describing the schema, for example, how or why it is used, or if only a subset of the schema is used.

### Data Field
This section is used to describe each data field in the dataset. It must be repeated for each data field. 
...
If you have already filled in an external schema, then it is not necessary to fill in the data fields as well. You may do so however if you want to call special attention to particular data fields
...
* Sensitivity - this indicates whether the data stored in this field is sensitive. Sensitive data might include personal information that could be harmful if exposed, such as social security numbers, personal addresses, or health information.
...
Here you can include an example of how the data typically appears in the Resource
...
The dataset may link to external resources. For example, to pages on Wikidata describing persons, or to images stored in an image bank. 
...
Here you can answer, with yes or no, the question 'Does the resource contain data that, if viewed directly, might be offensive, insulting, threatening, or might otherwise cause anxiety?'. If the answer is yes, then you can specify how and why
...
* Review - an indication, yes or no, if an ethical review was carrie dout
* Description - if an ethical review was carried out, this describes the outcomes of the ethical review, and how they influenced the dataset creation
...
## 3.8 Data Provenance
The dataset may have used one or more sources. For example, a set of digital scans of letters can refer to the original paper source of the letters. For each source, the following information may be filled in:

* Name - the name of the source
* URL - a URL of the source if available
* Description - a brief description of the source and why it was used. For example, "the original paper letters were digitally scanned to create the letters dataset"
...
Note that this does not refer to Data Provenance in terms of the chain of previous owners of a dataset.
...
* Motivating Factor(s) and Problem space(s) - What motivated the creation of this dataset? What were some of the reasons underlying the major choices involved in putting it together? Provide a description of the specific problem space that this dataset intends to address

## 4.1 Uses
* Dataset Use - one or more uses of the dataset can be selected from a vocabulary.
* Explain for special cases chosen above - If a special use has been selected, then this can be explained here.
* Links to Related Items
  * Publications - URL(s) of paper(s) introducing or describing the dataset
...
Datasets that are safe to use on their own may not be suitable to use with other data. For example, combining sets of anonymised persons may allow the persons to be identified.
...
All datasets are influenced by the choices of the human beings who creator them, whether this is by the selection of the data, the creation of a data model, the choice of a software algorithm etc. These choices are often made against a background of a particular research question, a particular use, the experience and interests of the creators, and their social, ethnic, cultural and gender backgrounds. 

The human perspective is related to bias, but it is different. Biases are already present and detected in the dataset. The choices made due to a human perspective may result in a dataset that is perfectly suited to their context but that has a serious bias when approached from a different domain and perspective. For example, think of an AI trained on product reviews in America by marketeers that is then applied to do sentiment detection in television programmes in the Netherlands by academic researchers. Knowledge of the human perspective can help in identifying the match with different domains and uses, and in asking critical questions about the suitability of a dataset for a particular purpose. 
...
* Annotator description - This contains a description of each annotator group. 
